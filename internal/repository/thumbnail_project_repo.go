...
func (r *ThumbnailProjectRepository) SaveSnapshot(ctx context.Context, workspaceID int64, projectID string, snapshot models.ThumbnailProjectSnapshot, createdBy int64) (*models.ThumbnailProjectSnapshotResult, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT id, project_id, revision_number, schema_version, snapshot_json, snapshot_sha256, renderer_version, created_by, created_at FROM thumbnail_project_revisions WHERE project_id = $1 AND snapshot_sha256 = $2`, projectID, hash).Scan(&existing.ID, &existing.ProjectID, &existing.RevisionNumber, &existing.SchemaVersion, &existing.SnapshotJSON, &existing.SnapshotSHA256, &existing.RendererVersion, &existing.CreatedBy, &existing.CreatedAt)
	if err == nil {
