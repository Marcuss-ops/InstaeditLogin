...
func TestPostRepository_UpdateReconcileStatusWithLease_CAS(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE post_targets SET status = $1, platform_post_id = $2, error_message = $3, published_at = $4, provider_state = $6, container_id = $7, reconcile_owner_id = NULL, reconcile_until = NULL, reconcile_heartbeat_at = NULL WHERE id = $5 AND status = 'publishing' AND reconcile_owner_id = $8 AND reconcile_until > NOW()`).
