...
//
//	dispatcher.go        — lifecycle (Run / drainOnce / processOne
//	                       orchestrator / safeProcess / heartbeatLoop / evID)
//	dispatcher_backoff.go — computeBackoff / pow2 (decorrelated-jitter retry)
//	dispatcher_mark.go    — the four mark* phase methods (processed /
//	                       terminal DLQ / max-attempts DLQ / failed-backoff)
package outbox
...
func (d *Dispatcher) processOne(ctx context.Context, ev *models.OutboxEvent) (err error) {
	...
	// contract). The four arms live in dispatcher_mark.go as
	// phase methods (markProcessed / markDeadLetterTerminal /
	// markDeadLetterMaxAttempts / markFailedBackoff).
	switch {
	case processErr == nil:
		return d.markProcessed(ev, leaseID, duration)
	case errors.Is(processErr, ErrTerminal):
		// Terminal error → DLQ regardless of attempt count.
		return d.markDeadLetterTerminal(ev, leaseID, duration, processErr)
	case ev.AttemptCount >= d.cfg.MaxAttempts:
		// Transient retries exhausted — DLQ.
		return d.markDeadLetterMaxAttempts(ev, leaseID, duration, processErr)
	default:
		// Transient failure — backoff and retry.
		return d.markFailedBackoff(ev, leaseID, duration, processErr)
	}
}
