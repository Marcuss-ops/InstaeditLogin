...
# `bash`; Tigris provisioning is retired. The canonical production object
# store is MinIO managed by Docker Compose on the VPS.
echo "Archived historical material; provision-tigris.sh is non-operational." >&2
exit 1

# HISTORICAL BODY COMMENTED — RETAINED FOR AUDIT ONLY; DO NOT COPY OR EXECUTE
# The original Tigris provisioning text follows as shell comments. MinIO
# on Docker Compose/VPS is the canonical production object-store path.
