...
case "${1:-}" in
  ...
  *)
    echo "usage: $0 [--audit|--apply|--ui-fallback]  (default: --dry-run)" >&2
    exit 1
    ;;
esac
...
# ─── Mode: --ui-fallback ──────────────────────────────────────────────────
if [[ "$mode" == "ui-fallback" ]]; then
  cat <<EOF
── Manual Fly Dashboard destruction ─────────────────────────────
Safety gate FIRST: \`curl $HEALTHCHECK_URL\` must return 200 BEFORE
...
1. https://fly.io/apps/$APP
   • Settings → "Delete App" (Danger Zone) → type 'instaedit-login' → confirm
...
if [[ "$mode" == "apply" ]]; then
  ...
  if [[ $failures -eq 0 ]]; then
    echo
    echo "Audit log saved:          $apply_log"
    echo "Chain-of-custody hash:    $(sha256sum "$apply_log" | awk '{print $1}')"
    exit 0
  else
    exit 5
  fi
