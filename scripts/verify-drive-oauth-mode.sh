#!/usr/bin/env bash
#
# scripts/verify-drive-oauth-mode.sh — Quick health check for a Google
# OAuth access token issued by InstaEdit for the Google Drive integration.
# Calls Google's public tokeninfo introspection endpoint and prints the
# aud / expires_in / scope / azp fields so an operator can confirm at a
# glance that the Drive grant carries the required scopes.
#
#   * `aud`  — which OAuth client_id the token was issued to
#              (compare against GOOGLE_DRIVE_CLIENT_ID in .env.production
...
#               Cross-check against docs/OAUTH-PRODUCTION.md.
...
#
# ─── USAGE ──────────────────────────────────────────────────────────────
#   ./scripts/verify-drive-oauth-mode.sh "$OAUTH_ACCESS_TOKEN"
#   # or pipe from a file:
#   ./scripts/verify-drive-oauth-mode.sh < token.txt
#   # or export:
#   GOOGLE_OAUTH_ACCESS_TOKEN=ya29.... ./scripts/verify-drive-oauth-mode.sh
#
# Exit codes:
...
# This avoids the trap where `./verify-drive-oauth-mode.sh -h` would
...
# non-zero exits.
BODY_FILE="$(mktemp -t verify-drive-oauth-mode.XXXXXX)"
...
# ─── Env-var cross-check: aud ↔ GOOGLE_DRIVE_CLIENT_ID ──────────────────
# The doc (docs/OAUTH-PRODUCTION.md) tells operators to sanity-check
# that aud matches GOOGLE_DRIVE_CLIENT_ID in .env.production. Do the
...
# strings. Skipped if GOOGLE_DRIVE_CLIENT_ID is not in the env (the
# script is also useful for ad-hoc token introspection).
if [[ -n "${GOOGLE_DRIVE_CLIENT_ID:-}" ]]; then
  if [[ "$aud" == "$GOOGLE_DRIVE_CLIENT_ID" ]]; then
    echo "✓ aud matches GOOGLE_DRIVE_CLIENT_ID from the environment."
  else
    echo "❌ aud mismatch! Token was issued by $aud," >&2
    echo "   but GOOGLE_DRIVE_CLIENT_ID in the env is $GOOGLE_DRIVE_CLIENT_ID." >&2
  ...
  echo "ℹ️  GOOGLE_DRIVE_CLIENT_ID not set in env; skipping aud cross-check."
...
# 3. expected scopes — soft cross-check against the Drive consent screen.
#    A Drive token used by InstaEdit must carry drive.readonly to list
#    and download files, and userinfo.profile to identify the account.
#    openid/email/profile are also present because the same OIDC flow
#    is reused; we only flag the two functional scopes here.
...
if ! grep -q "userinfo.profile" <<<"$scope"; then
  ...
  echo "   The token can authenticate but basic account identity may be unavailable." >&2
fi
