  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <button
        ...
        className={`group inline-flex h-8 items-center gap-1 rounded-lg border bg-black/25 px-1.5 transition-all hover:-translate-y-px hover:border-violet-300/60 hover:bg-violet-500/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70 disabled:cursor-progress disabled:opacity-50 ${error ? "border-red-300/70" : "border-white/[0.14]"}`}
      >
        <LanguageFlag code={value} className="h-4 w-6 rounded-[4px] ring-1 ring-white/15" />
        <ChevronDown size={12} className={`text-[#9aa0aa] transition-transform group-hover:text-white ${open ? "rotate-180" : ""}`} aria-hidden="true" />
      </button>
      ...
      {open ? (
        ...
        >
          {PICKER_OPTIONS.map(({ code, name }, index) => (
            ...
            >
              <LanguageFlag code={code} className="h-4 w-6 rounded-[4px] ring-1 ring-white/15" />
